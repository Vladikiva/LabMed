    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.option_general:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Главный врачl");
                return true;
            case R.id.option_Dentiste:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Дантист");
                return true;
            case R.id.option_Ophtalmologue:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Офтальмолог");
                return true;
            case R.id.option_ORL:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Лор");
                return true;
            case R.id.option_Pédiatre:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Педиатр");
                return true;
            case R.id.option_Radiologue:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Радиология");
                return true;
            case R.id.option_Rhumatologue:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Ревматолог");
                return true;
            default:
